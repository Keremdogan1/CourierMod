...
public class Waypoint extends WaypointBase<Waypoint>
{
    ...
    {
        super(modId, name);
        setPosition(dimension.getValue().toString(), position);
    }
    ...
    private BlockPos getInternalPosition(String targetDimension)
    {
        ...
        {
            if (this.dim.equalsIgnoreCase(World.NETHER.getValue().toString()))
            {
                // Convert coords to 8x horizontal scale outside of the Nether
                pos = new BlockPos(pos.getX() * 8, pos.getY(), pos.getZ() * 8);
            }
            else if (targetDimension.equalsIgnoreCase(World.NETHER.getValue().toString()))
            {
                // Convert coords to 1/8 horizontal scale for display in the Nether
                pos = BlockPos.ofFloored(pos.getX() / 8.0, pos.getY(), pos.getZ() / 8.0);
            }
        }
        return pos;
    }
