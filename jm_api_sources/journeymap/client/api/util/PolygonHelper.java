...
public class PolygonHelper
{
    ...
    public static Area createChunksArea(@Nonnull final Collection<ChunkPos> chunks)
    {
        ...
        {
            area.add(new Area(new Rectangle(chunkPos.getStartX(), chunkPos.getStartZ(), 16, 16)));
        }
        return area;
    }
    ...
    public static List<MapPolygonWithHoles> classifyAndGroup(@Nonnull final List<MapPolygon> polygons)
    {
        ...
        final List<Pair<MapPolygon, Area>> holeAreas = holes.stream()
                .map(hole -> new Pair<>(hole, toArea(hole)))
        ...
        for (final MapPolygon hull : hulls)
        {
            ...
            for (final Iterator<Pair<MapPolygon, Area>> iterator = holeAreas.iterator(); iterator.hasNext(); )
            {
                final Pair<MapPolygon, Area> holeArea = iterator.next();
                final Area intersection = new Area(hullArea);
                intersection.intersect(holeArea.getRight());
                if (!intersection.isEmpty())
                {
                    hullHoles.add(holeArea.getLeft());
                    iterator.remove();
                }
            }
