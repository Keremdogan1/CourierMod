...
public class Patcher2 {
    public static void main(String[] args) throws Exception {
        ...
        String kuryeAlOld = "private int takeMission(CommandContext<ServerCommandSource> context) {\\s+ServerPlayerEntity p = context\\.getSource\\(\\)\\.getPlayer\\(\\);\\s+if \\(p == null\\) return 0;.*?return 1;\\s+}";
        ...
        String taksiAlOld = "private int takeTaksiMission(CommandContext<ServerCommandSource> context) {\\s+ServerPlayerEntity p = context\\.getSource\\(\\)\\.getPlayer\\(\\);\\s+if \\(p == null\\) return 0;.*?return 1;\\s+}";
        ...
        content = content.replaceAll(kuryeAlOld, Matcher.quoteReplacement(kuryeAlNew));
        content = content.replaceAll(taksiAlOld, Matcher.quoteReplacement(taksiAlNew));

        Files.write(path, content.getBytes("UTF-8"));
        System.out.println("Patcher2 applied successfully.");
    }
}
